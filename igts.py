...
def TopDown(Multivar_TS, k, step, double=0):
    """
    ...
    :return: (splits,InformationGain,knee)
             ...
             Information Gain is a numpy array of floats of size n. The ith element of the
             array represents the information gain found for the first i segments.
    ...
    if Integ_TS.ndim == 1:
        Len_TS = Integ_TS.shape[0] # length is the number of time points
    else:
    ...
    for i in range(k):
        ...
        for j in list(range(0, Len_TS, step)):
            # Skip if the distance from the previous segment is less than 50,000
            if i > 0 and abs(j - tryTT[i-1]) < min_distance:
                continue
...
def DP_IGTS(Multivar_TS, k, step=1, double=1):
    """
...
def IG_Cal_Incremental(IG_old, Integ_TS, pos_old, new_pos):
    """
     ...
     :param Integ_TS: The integral (cumulative sum) of the time series t
    ...
    #print(f'entropy before change: {entropy_change}')
    # Give higher importance to values outside the range -2 to 2
    importance_factor = 1.5
    importance_weight = np.sum((Integ_TS[:, new_pos] < -3) | (Integ_TS[:, new_pos] > 3)) / Num_TS
    entropy_change *= (1 + importance_factor * importance_weight)
    #print(f'entropy after change: {entropy_change}')
