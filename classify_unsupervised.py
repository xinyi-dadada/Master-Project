...
class PCAandKMeans():
    ...
    def merge_figures(self, folder_path, rows=14, cols=3):
        # Plot the result figure all together
        # Get all image file paths from the folder, sorted to maintain order
        image_paths = sorted(
            [os.path.join(folder_path, img) for img in os.listdir(folder_path) if
        ...
        plt.savefig('~/thesis/figure/result_pca/all_result_pca.png')
...
folder_path = ...
pca_kmeans = PCAandKMeans()
pca_kmeans.merge_figures(folder_path, rows=14, cols=3)
